#!/usr/bin/env python3
"""
Demo version of the Laser Power GUI that works without connected instruments.
This version simulates instrument behavior for demonstration purposes.
"""
...
class DemoInstruments:
    """Simulated instruments for demo purposes."""
    
    class DemoPowerMeter:
        def __init__(self):
            self.sensorName = "S132C (Demo)"
            self.sensorSerialNumber = "191219201"
            self.sensorType = "Photodiode sensor"
            self.connected = True
        
        def updatePowerReading(self, delay):
            time.sleep(delay)
            # Simulate power reading with some noise
            base_power = random.uniform(0.1, 0.5) * 1e-6  # µW range
            return base_power
        
        @property
        def meterPowerReading(self):
            return random.uniform(0.1, 0.5) * 1e-6
    ...
    class DemoLaser:
        ...
        def get_identity(self):
            return "Thorlabs,ITC4001,M01093719,1.0.0 (Demo)"
        ...
        def get_actual_current(self):
            ...
            return self.current_current + random.uniform(-2, 2)
        
        def set_output(self, enabled):
            self.output_enabled = enabled
        
        def ramp_current(self, target_ma, step_ma=20, delay_s=0.1):
            # Simulate ramping
            while abs(self.current_current - target_ma) > step_ma:
                if target_ma > self.current_current:
                    self.current_current += step_ma
                else:
                    self.current_current -= step_ma
                time.sleep(delay_s)
            self.current_current = target_ma
...
class LaserPowerGUIDemo:
    """Demo GUI for laser current vs power measurements."""
    
    def __init__(self, root):
        self.root = root
        self.root.title("Thorlabs Laser Current vs Power Measurement (DEMO)")
        self.root.geometry("900x750")
        
        # Demo instruments
        self.power_meter = DemoInstruments.DemoPowerMeter()
        self.laser = DemoInstruments.DemoLaser()
    ...
    def show_demo_notice(self):
        ...
        messagebox.showinfo("Demo Mode", 
                           ...
                           "• All measurements are fake data\n"
                           "• Use this to test the interface\n\n"
                           "For real measurements, use laser_power_gui.py")
    
    def setup_gui(self):
        """Create the GUI layout."""
        # Add demo indicator
        demo_frame = tk.Frame(self.root, bg='orange')
        demo_frame.pack(fill=tk.X)
        demo_label = tk.Label(demo_frame, text="🚧 DEMO MODE - Simulated Instruments 🚧", 
    ...
    def setup_sweep_tab(self):
        ...
        self.readings_var = tk.StringVar(value="3")  # Faster for demo
        ...
        self.stab_time_var = tk.StringVar(value="0.5")  # Faster for demo
        stab_spin = ttk.Spinbox(settings_frame, from_=0.1, to=5.0, increment=0.1, width=5, textvariable=self.stab_time_var)
        ...
        self.start_sweep_btn = ttk.Button(button_frame, text="Start Sweep Measurement", 
                                         command=self.start_sweep_measurement)
        ...
        # Configure columns to match your table image
    ...
    def setup_manual_tab(self):
        ...
        ttk.Label(status_frame, textvariable=self.actual_current_var, 
                 foreground='blue', font=('Arial', 10, 'bold')).pack(side=tk.LEFT, padx=5)
        ...
        ttk.Label(power_frame, textvariable=self.optical_power_var, 
                 font=('Arial', 14, 'bold'), foreground='red').pack(side=tk.LEFT, padx=5)
    ...
    def setup_status_tab(self):
        ...
        # Demo notice
        demo_notice = tk.Label(self.status_frame, 
                              text="⚠️ DEMO MODE: All instruments are simulated ⚠️",
                              bg='yellow', fg='black', font=('Arial', 12, 'bold'))
        demo_notice.pack(fill=tk.X, padx=20, pady=10)
        ...
        pm_status_label = ttk.Label(pm_frame, text="Connected - S132C (Demo)", foreground='green')
        pm_status_label.pack(side=tk.LEFT, padx=5)
        ...
        laser_status_label = ttk.Label(laser_frame, text="Connected - ITC4001 (Demo)", foreground='green')
        laser_status_label.pack(side=tk.LEFT, padx=5)
        ...
        # Add demo instrument info
        demo_info = """DEMO MODE - Simulated Instruments

Power Meter:
- Model: S132C (Demo)
- Serial: 191219201
- Type: Photodiode sensor
- Wavelength: 1550 nm
...
All measurements are simulated data.
For real measurements, use the full version
with connected instruments.
"""
        info_text.insert(tk.END, demo_info)
        ...
        emergency_btn = ttk.Button(emergency_frame, text="EMERGENCY STOP (Demo)", 
    ...
    def run_sweep_measurement(self):
        """Run the automated sweep measurement (demo version)."""
        try:
            ...
            for i, current_ma in enumerate(self.current_points):
                ...
                self.laser.ramp_current(current_ma, step_ma=50, delay_s=0.05)  # Faster for demo
                ...
                for j in range(readings_per_point):
                    if not self.sweep_running:
                        break
                    # Simulate realistic power vs current relationship
                    # Power increases roughly with current but with noise
                    base_power = (current_ma / 1000.0) * random.uniform(0.8, 1.2) * 1e-3  # mW range
                    power_readings.append(base_power)
                    time.sleep(0.1)
            ...
            # Simulate laser shutdown
            self.laser.set_output(False)
            self.laser.ramp_current(0, step_ma=50, delay_s=0.02)
            
            # Update UI
            self.root.after(0, self.sweep_measurement_complete)
            
        except Exception as e:
            self.root.after(0, lambda: messagebox.showerror("Sweep Error", f"Demo measurement failed: {str(e)}"))
            self.root.after(0, self.sweep_measurement_complete)
    ...
    def sweep_measurement_complete(self):
        ...
        self.sweep_status_var.set(f"Demo sweep complete - {len(self.sweep_data)} points measured")
    
    def set_manual_current(self):
        """Set laser current manually (demo)."""
        try:
    ...
    def toggle_laser_output(self):
        """Toggle laser output on/off (demo)."""
    ...
    def take_single_measurement(self):
        """Take a single power measurement (demo)."""
        ...
        try:
            ...
            for i in range(avg_count):
                current = self.laser.get_actual_current()
                # Simulate power based on current
                power = (current / 1000.0) * random.uniform(0.8, 1.2) * 1e-3
            ...
            messagebox.showerror("Error", f"Demo measurement failed: {str(e)}")
    ...
    def export_sweep_data(self):
        ...
        filename = filedialog.asksaveasfilename(
            ...
            title="Save Demo Sweep Data",
            initialvalue="demo_sweep_data.csv"
        )
        ...
        if filename:
            try:
                ...
                messagebox.showinfo("Success", f"Demo data exported to {filename}")
    ...
    def export_manual_data(self):
        ...
        filename = filedialog.asksaveasfilename(
            defaultextension=".csv",
            filetypes=[("CSV files", "*.csv"), ("All files", "*.*")],
            title="Save Demo Manual Data",
            initialvalue="demo_manual_data.csv"
        )
        ...
        if filename:
            try:
                ...
                messagebox.showinfo("Success", f"Demo manual data exported to {filename}")
    ...
    def emergency_stop(self):
        ...
        self.sweep_running = False
        messagebox.showinfo("Demo Emergency Stop", "Demo laser safely stopped")
...
def main():
    """Main function to run the demo GUI."""
    root = tk.Tk()
    app = LaserPowerGUIDemo(root)
